...
webodm_dialog.py
================
Dialog for SkyStitch's "WebODM (remote)" engine: point at a WebODM server
(local native installer or remote), upload a photo folder, process there, and
download the results into QGIS -- no Docker on this machine.

Same UI conventions as odm_dialog.py (scrollable form; pinned log/progress/
buttons; background QgsTask + signals).
"""
...
from .webodm_worker import WebODMTask
from .pipeline.webodm_client import WebODMClient, find_images, requests_available

SETTINGS_GROUP = "SkyStitchWebODM"
PC_QUALITY = [
...
class WebODMDialog(QDialog):
    ...
    def __init__(self, iface, parent=None):
        ...
        self.setWindowTitle("SkyStitch - 3D Photogrammetry (WebODM, no Docker)")
    ...
    def _build_ui(self):
        ...
        info = QLabel(
            "Process photos on a WebODM server (local native installer or remote) and "
            "load the results into QGIS -- no Docker needed on this computer. Get the free "
            "Windows installer at webodm.org/download; it runs WebODM natively."
        )
        info.setWordWrap(True)
        layout.addWidget(info)

        # ---- server ----
        srv_box = QGroupBox("WebODM server")
        srv = QFormLayout(srv_box)
        srv.setLabelAlignment(Qt.AlignmentFlag.AlignRight)
        self.url_edit = QLineEdit("http://localhost:8000")
        self.url_edit.setToolTip("WebODM address. The native Windows installer usually serves at http://localhost:8000")
        srv.addRow("Server URL:", self.url_edit)
        self.user_edit = QLineEdit("admin")
        srv.addRow("Username:", self.user_edit)
        self.pass_edit = QLineEdit()
        self.pass_edit.setEchoMode(QLineEdit.EchoMode.Password)
        srv.addRow("Password:", self.pass_edit)
        ...
        self.gcp_widget.setDialogTitle("Optional WebODM/ODM gcp_list.txt")
        opt.addRow("GCP list (optional):", self.gcp_widget)
        layout.addWidget(opt_box)

        # ---- assets to download / load ----
        dl_box = QGroupBox("Download & load")
        dl = QFormLayout(dl_box)
        dl.setLabelAlignment(Qt.AlignmentFlag.AlignRight)
        self.dl_ortho = QCheckBox("Orthophoto")
        self.dl_ortho.setChecked(True)
        self.dl_dsm = QCheckBox("DSM")
        self.dl_dsm.setChecked(True)
        self.dl_dtm = QCheckBox("DTM")
        self.dl_dtm.setChecked(True)
        self.dl_pc = QCheckBox("Point cloud")
        self.dl_pc.setChecked(True)
        self.dl_mesh = QCheckBox("3D mesh (zip)")
        a_row = QHBoxLayout()
        for w in (self.dl_ortho, self.dl_dsm, self.dl_dtm, self.dl_pc, self.dl_mesh):
            a_row.addWidget(w)
        a_row.addStretch()
        dl.addRow("Download:", a_row)
        self.load_check = QCheckBox("Load rasters/point cloud into the QGIS canvas")
        self.load_check.setChecked(True)
        dl.addRow("", self.load_check)
        ...
        brow = QHBoxLayout()
        self.run_button = QPushButton("Process on WebODM")
        ...
        brow.addWidget(self.open_folder_button)
        brow.addStretch()
    ...
    def _restore_settings(self):
        ...
        self.url_edit.setText(s.value("url", "http://localhost:8000", type=str) or "http://localhost:8000")
        self.user_edit.setText(s.value("user", "admin", type=str) or "admin")
    ...
    def _client_from_form(self):
        if not requests_available():
            ...
            return None
        return WebODMClient(self.url_edit.text().strip(),
    ...
    def _on_test_connection(self):
        ...
        if client is None:
            return
        self._log("[INFO] Testing connection to %s ..." % self.url_edit.text().strip())
        ...
        self._log("[INFO] " + msg)
        if ok:
            QMessageBox.information(self, "WebODM OK", msg)
        else:
            QMessageBox.warning(self, "WebODM connection failed", msg)

    def _on_run_clicked(self):
        if self._client_from_form() is None:
            return
        ...
        if len(find_images(input_dir)) < 2:
            QMessageBox.warning(self, "Not enough photos", "WebODM needs several overlapping photos (typically 20+).")
            return
        ...
        download = []
        if self.dl_ortho.isChecked():
            download.append("orthophoto")
        if self.dl_dsm.isChecked():
            download.append("dsm")
        if self.dl_dtm.isChecked():
            download.append("dtm")
        if self.dl_pc.isChecked():
            download.append("point_cloud")
        if self.dl_mesh.isChecked():
            download.append("mesh")
        ...
        self.task = WebODMTask(
            self.url_edit.text().strip(), self.user_edit.text().strip(), self.pass_edit.text(),
    ...
    def _on_cancel_clicked(self):
        if self.task:
            self.task.cancel()
        self.cancel_button.setEnabled(False)
        self._log("\nCanceling... (asking WebODM to stop the task)")
    ...
    def _on_finished_ok(self, outputs):
        ...
        self.iface.messageBar().pushSuccess("SkyStitch - WebODM", "Finished. Results saved to %s" % self.last_output_dir)
    ...
    def _on_finished_error(self, message):
        ...
        self.run_button.setEnabled(True)
        self.cancel_button.setEnabled(False)
        QMessageBox.critical(self, "WebODM processing failed", message)
